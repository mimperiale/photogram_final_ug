class PhotosController < ApplicationController
  before_action :current_user_must_be_photo_poster, :only => [:edit_form, :update_row, :destroy_row]
  ...
  def index
    ...
    @photos = @q.result(:distinct => true).includes(:likes, :comments, :poster, :fans).page(params[:page]).per(10)
